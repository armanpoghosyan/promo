                ...
                ->chunk(500, function ($receipts) use ($handle) {
                    foreach ($receipts as $receipt) {
                        fputcsv($handle, [
                            ...
                            $receipt->is_suspicious ? 'yes' : 'no',
                            implode(', ', $receipt->suspicious_reasons ?? []),
                            $receipt->submitted_at?->toDateTimeString(),
                            $receipt->verified_at?->toDateTimeString(),
                            $receipt->verifiedBy?->email,
            ...
            DrawWinner::query()
                ...
                ->chunk(500, function ($winners) use ($handle) {
                    foreach ($winners as $winner) {
                        $participant = $winner->receipt?->participant;
            ...
            Draw::query()
                ...
                ->chunk(100, function ($draws) use ($handle) {
                    foreach ($draws as $draw) {
                        ...
                        $activeWinners = $selected + $contacting + $confirmed;
                        ...
                        foreach ($draw->drawPrizes as $drawPrize) {
                            fputcsv($handle, [
                                ...
                                $draw->randomized_at?->toDateTimeString(),
                            ]);
                        }
                    }
                });
